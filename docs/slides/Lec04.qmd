...
We can coerce a list into an atomic vector using `unlist()` - type coercion rules then apply to determine the resulting type.
...
Attributes are metadata that can be attached to objects in R. Some are special ( e.g. `class`, `comment`, `dim`, `dimnames`, `names`, ...) and they change the behavior of the object(s).
