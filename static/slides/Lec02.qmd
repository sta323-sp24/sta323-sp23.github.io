...
Because `NA`s represent missing values it makes sense that any calculation using them will also be missing.
...
In R `&` and `|` are almost always going to be the right choice, the only time you would use `&&` or `||` is when you need to take advantage of [short-circuit evaluation](https://en.wikipedia.org/wiki/Short-circuit_evaluation).


::: {.aside}
**Note** previously (before R 4.3) both `||` and `&&` only use the *first* value in the vector, all other values are ignored, there was no warning about the ignored values.
:::
